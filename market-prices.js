(() => {
  ...
  const card = ([category, name], extra = false) => `<article class="price-card catalog-price${extra ? ' catalog-extra' : ''}" data-product="${name}"><div class="card-top"><span class="pill">${category}</span><span class="origin">승인 재고 합산</span></div><div class="product-image"></div><h3>${name}</h3><p>판매자 승인 재고 · 0.5kg 단위</p><div class="price"><strong>재고 확인 중</strong><span class="up">실시간 합산</span></div><div class="bar-row"><span>판매자 등록가 기준</span><span>kg 기준</span></div><div class="price-bar"><i style="width:0%"></i></div><button class="add-cart" disabled>재고 확인 중 <b>—</b></button></article>`;
