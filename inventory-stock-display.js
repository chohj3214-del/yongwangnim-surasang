(() => {
  ...
  function updateCatalogStock() {
    ...
    document.querySelectorAll('.market-grid .catalog-price').forEach(card => {
      ...
      else { if (price) price.textContent = money(Number(selectedItem.wholesale_price) * purchaseQuantity); button.disabled = false; button.innerHTML = `${purchaseQuantity}kg 장바구니 담기 <b>+</b>`; }
    });
  ...
  document.addEventListener('change', event => { const card = event.target.closest('.catalog-price'); if (!card) return; if (event.target.matches('.seafood-state-select,.seafood-process-select')) { card.dataset.selectedInventoryId = ''; card.dataset.purchaseQuantity = '.5'; } else if (event.target.matches('.seafood-price-select')) { card.dataset.selectedInventoryId = event.target.value; card.dataset.purchaseQuantity = '.5'; } else if (event.target.matches('.catalog-quantity-input')) { const max = Number(event.target.max), value = Math.min(max, Math.max(.5, Math.round(Number(event.target.value || .5) * 2) / 2)); card.dataset.purchaseQuantity = String(value); } else return; updateCatalogStock(); window.dispatchEvent(new Event('catalog-price-selection-changed')); });
