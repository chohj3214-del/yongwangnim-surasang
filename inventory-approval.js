(() => {
  ...
  window.openAccount=async()=>{
    ...
    try{
      ...
      const details=item=>`<div><b>${esc(item.product_name)}</b><small>${esc(item.seller_name)} · ${item.quantity}g · ${esc(item.location)}</small><em>500g당 ₩ ${Number(item.wholesale_price).toLocaleString()}</em></div>`;
      ...
      showModal(`<h2>관리자 상품 관리</h2><p>승인된 재고만 구매자 화면에 공개됩니다. 모든 재고는 g, 가격은 500g 기준입니다.</p><div class="approval-summary"><span>승인 대기 <b>${pending.length}건</b></span><span>판매 중 <b>${approved.length}건</b></span><span>비승인 <b>${rejected.length}건</b></span></div><h3 class="admin-section-title">승인 대기 재고</h3><div class="approval-list">${pending.map(row).join('')||'<p>승인 대기 재고가 없습니다.</p>'}</div><h3 class="admin-section-title">판매 중 재고</h3><div class="approval-list approved">${approved.map(item=>`<article>${details(item)}<div class="approval-actions"><i>승인 완료</i>${deleteButton(item)}</div></article>`).join('')||'<p>판매 중 재고가 없습니다.</p>'}</div><h3 class="admin-section-title">비승인 재고</h3><div class="approval-list rejected">${rejected.map(item=>`<article>${details(item)}<div class="approval-actions"><button onclick="approveInventory('${item.id}')">재승인</button>${deleteButton(item)}</div></article>`).join('')||'<p>비승인 재고가 없습니다.</p>'}</div><button class="logout-button" onclick="logout()">관리자 로그아웃</button>`);
